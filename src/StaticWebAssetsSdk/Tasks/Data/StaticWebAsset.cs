...
#endif
{
    ...
    public void ApplyDefaults()
    {
        ...
        if (string.IsNullOrEmpty(Fingerprint) || string.IsNullOrEmpty(Integrity) || FileLength == -1 || LastWriteTime == DateTimeOffset.MinValue)
        {
            ...
            (Fingerprint, Integrity) = DeferredFingerprint == bool.TrueString ? (string.Empty, string.Empty) : ComputeFingerprintAndIntegrity(file);
    ...
    {
        var file = ResolveFile(Identity, OriginalItemSpec);
        return ComputeFingerprintAndIntegrity(file);
    }
